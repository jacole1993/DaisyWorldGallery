...
import com.daisyworld.web.rest.errors.BadRequestAlertException;
import java.net.URI;
...
/**
 * REST controller for managing {@link com.daisyworld.domain.Video}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Video> result = videoRepository
            ...
            .map(existingVideo -> {
                ...
                if (video.getVideo() != null) {
                    existingVideo.setVideo(video.getVideo());
                }
                if (video.getVideoContentType() != null) {
                    existingVideo.setVideoContentType(video.getVideoContentType());
                }
                if (video.getDateTaken() != null) {
                    existingVideo.setDateTaken(video.getDateTaken());
                }
                if (video.getIsMomInPicture() != null) {
                    existingVideo.setIsMomInPicture(video.getIsMomInPicture());
                }
                if (video.getIsDadInPicture() != null) {
                    existingVideo.setIsDadInPicture(video.getIsDadInPicture());
                }
                if (video.getIsFamilyInPicture() != null) {
                    existingVideo.setIsFamilyInPicture(video.getIsFamilyInPicture());
                }
